...
urls={
    ...
    'Onedrive':'7yrgt9uvq77ionw',
    'Word':'myg3tvu5e31c64q',
    'Excel':'w4q6tzu3r3ra7yp',
    'OneNote':'myg3tvukdwwh64q',
    'Bing':'ky51t5u28r0h74x',
    'Microsoft Launcher':'2yeztpu8m79mury',
}
...
# 删除表中数据
sql='truncate table database_comments_vivo;'
cursor.execute(sql)   
db.commit()
...
def scrapy_table(page):
    ...
    for table_num in range(1, vrows):
        ...
        print(data)
        sql = 'insert into database_comments_vivo(date,user,content,score,version,name) values(%s,%s,%s,%s,%s,%s);'
        cursor.execute(sql,data)     # 插入数据
        db.commit()


for k,v in urls.items():
    driver.get("https://app.diandian.com/app/"+str(v)+"/android-review?market=4&summer=")
    # 爬前10页
    for i in range(10):
        end = scrapy_table(i)
        if end == 3:
